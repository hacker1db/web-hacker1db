 ...
 */
export function processShortcodes(content: string, frontmatter: PostMatter): string {
  let processedContent = content;

  // Process {{< param key >}} shortcodes
  processedContent = processedContent.replace(/\{\{<\s*param\s+(\w+)\s*>\}\}/g, (match, paramKey) => {
  ...
  processedContent = processedContent.replace(/\{\{<\s*youtube\s+([^\s>]+)\s*>\}\}/g, (match, videoId) => {
    return `
<div style="position: relative; width: 100%; height: 0; padding-bottom: 56.25%; margin: 2rem 0;">
  ...
  processedContent = processedContent.replace(/\{\{<\s*vimeo\s+([^\s>]+)\s*>\}\}/g, (match, videoId) => {
    return `
<div style="position: relative; width: 100%; height: 0; padding-bottom: 56.25%; margin: 2rem 0;">
  <iframe 
    style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: 0; border-radius: 8px;"
  ...
  processedContent = processedContent.replace(/\{\{<\s*newsletter\s*>\}\}/g, () => {
    return `
...
<figure style="margin: 2rem 0; text-align: center;">
  ...
  processedContent = processedContent.replace(/\{\{<\s*gist\s+([^\s]+)\s+([^\s>]+)\s*>\}\}/g, (match, username, gistId) => {
    return `
<div style="margin: 2rem 0; padding: 1rem; border: 1px solid #374151; border-radius: 8px; background-color: #1f2937;">
  ...
  processedContent = processedContent.replace(/\{\{<\s*ref\s+"([^"]+)"\s*>\}\}/g, (match, path) => {
  ...
  processedContent = processedContent.replace(/\{\{<\s*relref\s+"([^"]+)"\s*>\}\}/g, (match, path) => {
    const cleanPath = path.replace(/\.md$/, '').replace(/^\//, '');
    return `/posts/${cleanPath}`;
  });
